...
Existing external-services network (already created on your host)
...
Create a new tunnel (give it a name like "nextcloud")
Copy the tunnel token (you'll need this for the .env file)
...
.env.template
README.md
Important: Create a .env file locally (DO NOT commit this to Git!)
...
Set repository reference (branch): main or master
Specify Compose path: docker-compose.yml
...
Wait for all containers to start (check logs in Portainer)
Access Nextcloud through your domain (e.g., https://cloud.example.com)
Create an admin account on first access
Nextcloud will automatically configure itself with PostgreSQL and Redis
...
Portainer will pull the latest changes from Git and redeploy
Troubleshooting
Checking Logs
